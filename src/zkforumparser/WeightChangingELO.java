 ...
 * @author User
 */
public class WeightChangingELO {
    
    Map<Integer, Double> ratings = new HashMap();
    
    double K = 64;

    public WeightChangingELO(){
        
    }
    
    public WeightChangingELO(double k){
        this.K = k;
    }
    ...
    public void evaluateResult(Collection<Collection<Integer>> winnerTeams, Collection<Collection<Integer>> loserTeams, float weight) {
        ...
        winners.forEach(x -> ratings.put(x, ratings.get(x) + weight * Math.sqrt((winners.size() + losers.size()) / 2d) * (K - K / (1 + Math.pow(10, -dif / 400d))) / winners.size()));
        losers.forEach(x -> ratings.put(x, ratings.get(x) - weight * Math.sqrt((winners.size() + losers.size()) / 2d) * (K / (1 + Math.pow(10, dif / 400d))) / losers.size()));
    }
