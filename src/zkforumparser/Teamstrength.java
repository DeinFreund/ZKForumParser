...
public class Teamstrength extends ELO {
    ...
    public List<Double> predictResult(List<Collection<Integer>> teams) {
        return teams.stream().map(t -> (t.stream().mapToDouble(x -> (Math.pow(10,ratings.get(x)/400))).reduce(0.0, Double::sum) / teams.stream().flatMap(x -> x.stream()).mapToDouble(x -> (Math.pow(10,ratings.get(x)/400))).reduce(0.0, Double::sum) )).collect(Collectors.toList());
    }
//------------
    ...
    public double predictResult(List<Collection<Integer>> teams, Collection<Integer> team) {
        return (team.stream().mapToDouble(x -> (Math.pow(10,ratings.get(x)/400))).reduce(0.0, Double::sum) / teams.stream().flatMap(x -> x.stream()).mapToDouble(x -> (Math.pow(10,ratings.get(x)/400))).reduce(0.0, Double::sum));
    }
//------------
